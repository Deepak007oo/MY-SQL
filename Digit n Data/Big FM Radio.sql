...
select count(awardid) as 'No of Awards',showname
from awards a 
...
select showname
from shows s 
join showpartnerships sp on sp.showid = s.showid
...
from hosts h
join shows s on s.hostid = h.hostid;
...
select HostName, ShowCount,
dense_rank () OVER (order BY ShowCount desc) as rnk
from Hosts;
